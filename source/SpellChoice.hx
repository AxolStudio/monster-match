		...
		FlxG.camera.flash(FlxColor.WHITE, .1, function()
		{
			...
			FlxTween.tween(back, {alpha:1}, .2,
			{
				type:FlxTweenType.ONESHOT, ease:FlxEase.sineIn, onComplete:function(_)
				{
					FlxTween.num(0, 1, .33,
					{
						ease:FlxEase.backOut, type:FlxTweenType.ONESHOT, onComplete:function(_)
						{
							ready = true;
						}
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (ready)
		{
			...
			if (over !=-1 && FlxG.mouse.justReleased)
			{
				...
				FlxTween.tween(cards[over], {alpha:0, y: -60}, .4, {type:FlxTweenType.ONESHOT, startDelay:.2, onComplete:function(_)
				{
