...
class PauseScreen extends FlxSubState
{
	...
	override public function create():Void 
	{
		...
		FlxTween.num(0, 1, .2, {type:FlxTweenType.ONESHOT, ease:FlxEase.circOut, onComplete:function(_){
			ready = true;
	...
	private function resumeGame():Void
	{
		...
		ready = false;
		Sounds.play("click", .2);
		FlxTween.num(1, 0, .2, {type:FlxTweenType.ONESHOT, ease:FlxEase.circOut, onComplete:function(_){
			close();
		}}, function(Value:Float){
			for (m in infos)
				m.alpha = Value;
