...
class PlayState extends FlxState
{
    ...
    public function doCombat(elapsed:Float):Void
    {
        if ((heroReady || currentHero == -1) && (monsterReady || currentMonster == -1))
        {
            ...
            if (fightCounter >= TIME_BETWEEN_FIGHT)
            {
                ...
                {
                    heroReady = false;
                    FlxTween.tween(fightingHero, {x: fightingHero.x - 4}, .5, {
                        type: FlxTweenType.ONESHOT,
                        ease: FlxEase.backIn,
                        onComplete: function(_)
                        {
                ...
                else
                {
                    ...
                    else if (heroReady && monsterReady)
                    {
                        ...
                        FlxTween.tween(fightingMonster, {x: fightingMonster.x + 4}, .5, {
                            type: FlxTweenType.ONESHOT,
                            ease: FlxEase.backIn,
                            onComplete: function(_)
                            {
                                ...
                                else
                                {
                                    // KILL THE MONSTER!
                                    Sounds.play("death");
                                    FlxTween.color(fightingMonster, .5, FlxColor.WHITE, FlxColor.fromRGB(255, 0, 0, 0), {
                                        type: FlxTweenType.ONESHOT,
                                        ease: FlxEase.circInOut,
                                        onComplete: function(_)
                                        {
                        ...
                        FlxTween.tween(fightingHero, {x: fightingHero.x - 4}, .5, {
                            ...
                            onComplete: function(_)
                            {
                                ...
                                else
                                {
                                    ...
                                    showHeroSkull();
                                    FlxTween.color(fightingHero, .5, FlxColor.WHITE, FlxColor.fromRGB(255, 0, 0, 0), {
                                        type: FlxTweenType.ONESHOT,
                                        ease: FlxEase.circInOut,
                                        onComplete: function(_)
                                        {
    ...
    public function showHeroSkull():Void
    {
        ...
        FlxTween.tween(heroSkull, {y: HEROSTARTY - 16}, 2, {
            type: FlxTweenType.ONESHOT,
    ...
    private function pauseGame():Void
    {
        ...
        ready = false;
        Sounds.play("click", .2);
        openSubState(new PauseScreen(currentEnv, function()
        {
            ready = true;
        }));
    }
