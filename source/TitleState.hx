...
class TitleState extends FlxState
{
	...
	override public function create():Void 
	{
		...
		FlxTween.tween(titleEffect, {y: -3}, 2, { type:FlxTweenType.ONESHOT, ease:FlxEase.sineInOut, onComplete:function(_){
			FlxG.camera.flash(0x66ffff00, .2, function() {
				Sounds.play("thunder_01", .33);
				FlxG.camera.flash(0x33ffff00, .2);
			});
			FlxTween.num(0, 1, 1, {type:FlxTweenType.ONESHOT, ease:FlxEase.sineIn, startDelay:.2, onComplete:function(_) {
				Sounds.play("thunder_02", .5);
				FlxG.camera.flash(0x99ffff00, .4);
				FlxTween.num(0, 1, .33, {type:FlxTweenType.ONESHOT, ease:FlxEase.sineOut, onComplete:function(_){
					ready = true;
